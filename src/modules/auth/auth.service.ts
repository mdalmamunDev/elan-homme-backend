...
const validateUserStatus = (user: TUser) => {
  if (user.status === 'inactive')
    throw new ApiError(
      ...
      'Your account has been inactivated. Please contact support',
    );
...
const createUser = async (userData: any) => {
  ...
  // Send admin notification for user signup
  await NotificationService.sendNotificationToSuperAdmin({
    title: 'New User Signup',
    message: `A new user ${user.name} has signed up with email ${user.email}`,
    type: 'account',
  });
...
const login = async (email: string, password: string, role: string) => {
  ...
  // Send admin notification if role is admin or super admin
  if (role === 'admin' || role === 'super-admin') {
    await NotificationService.sendNotificationToSuperAdmin({
      title: 'Admin Login',
      message: `Admin ${user.name} has logged in`,
      type: 'account',
    });
  }
...
const changePassword = async (
  ...
  // Send admin notification if user is admin or super admin
  if (user.role === 'admin') {
    await NotificationService.sendNotificationToSuperAdmin({
      title: 'Admin Password Changed',
      message: `Admin ${user.name} has changed their password`,
      type: 'account',
    });
  }
